...
public class NowplayerJrActivity extends FragmentActivity implements
		AsyncHttpCallback, MyAlertDialog.Callback, AppInfo.DownloadInfoCallback {

	private final static String TAG = NowplayerJrActivity.class.getSimpleName();
	...
	private ActionBar.Tab liveTab, programsTab, movieTab, veTab, faqTab,
			settingTab, interactiveTab;
	/**
...
//		 promptTermsAndCondition();
		...
		if (NowIDLoginStatus.getInstance().isLoggedIn()
				&& NowIDLoginStatus.getInstance().getNowID() != null) {
			UserSetting us2 = new UserSetting(appContext);
			us2.save(UserSettingType.LOGIN_NOWID); // save user setting to push
													// server
		}
		...
		try {
			...
			pixelLog = new PixelLogService(Constants.PIXEL_LOG_URL,
					Constants.PIXEL_LOG_APP_NAME,
					LVMediaPlayer.getUniqueIdentifier(this), NowIDLoginStatus
							.getInstance().getNowID(), NowIDLoginStatus
							.getInstance().getFsa());
	...
	public void setupActionbar() {
		...
		liveFragment = new LiveChannelFragment();
	...
	public void onBackPressed() {
		...
		} else if (lastFragment instanceof LiveChannelFragment) {
			if (((LiveChannelFragment) lastFragment).isfullscreen()) {
	...
	public void TabItemTapped(View imageButton) {
		...
		for (int i = 0; i < tabbar.getChildCount(); i++) {
			...
			LogUtils2.e("--------------------"+(tabitem_btn.isSelected())+"   i=="+i);
			...
			if (imageButton.isSelected()) { // //////////////////////////
				...
				if ((title.getText().toString())
					...
					fragmentTransaction.replace(R.id.fragment_container,
							liveFragment);
//					pixelLog.pixelLogOnTabPress(PixelLogService.PIXELLOG_LIVE,
//							version);
					...
					fragmentTransaction.replace(R.id.fragment_container,
							programsFragment);
//					pixelLog.pixelLogOnTabPress(PixelLogService.PIXELLOG_PROG,
//							version);
				} else if ((title.getText().toString())
						.equalsIgnoreCase(LanguageHelper
								.getLocalizedString("tabbar.item.movie.title"))) {
					fragmentTransaction.replace(R.id.fragment_container,
							movieFragment);
//					pixelLog.pixelLogOnTabPress(PixelLogService.PIXELLOG_MOVIE,
//							version);
				} else if ((title.getText().toString())
						.equalsIgnoreCase(LanguageHelper
								.getLocalizedString("tabbar.item.userguide.title"))) {
					fragmentTransaction.replace(R.id.fragment_container,
							faqFragment);
				} else if ((title.getText().toString())
						.equalsIgnoreCase(LanguageHelper
...
//							PixelLogService.PIXELLOG_INTERACTIVE, version);
						...
						.equalsIgnoreCase(getString(R.string.tab_jrclub_title))) {
					fragmentTransaction.replace(R.id.fragment_container,
							jrClubFragment);
//					pixelLog.pixelLogOnTabPress(PixelLogService.PIXELLOG_CLUB,
//							version);
				}
				// fragmentTransaction.replace(R.id.fragment_container,
				// faqFragment);
